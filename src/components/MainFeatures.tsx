...
const mainFeatures: Array<{
...
}> = [
  {
    ...
    description: 'Easily find individuals or companies with a history of fraud and corruption using advanced keyword and filter systems.'
  },
  {
      ...
      <div className='grid grid-cols-1 sm:grid-cols-3 mt-6 sm:mt-11 gap-4'>
        {mainFeatures.map((feature, index) => (
          <div key={index} className='py-6 px-4 border-gray20 border-1 rounded-lg'>
            ...
            <p className='mt-3 text-sm text-textGrayBold font-normal'>{feature.description}</p>
          </div>
        ))}
      </div>
    </div>
